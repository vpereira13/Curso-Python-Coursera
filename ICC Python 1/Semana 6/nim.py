...
def boasVindas():
	...
	while(opcao != 1 and opcao != 2):
		...
		opcao = int(input("Tipo de jogo: "))

	return opcao

def computador_escolhe_jogada(npecas, limite):
	if npecas <= limite:
		if npecas == 1:
			print("O computador tirou uma peça.")
		else:
			print("O computador tirou %d peças." % (npecas))
		return npecas
	else:
		if npecas <= limite*2:
			for i in range(1, limite+1):
				if npecas - i > limite:
					print("O computador tirou %d peças." % (i))
					return i

	return limite

def usuario_escolhe_jogada(npecas, limite):
	n = int(input("Quantas peças você vai tirar? "))

	while n > limite or n < 1 or n > npecas:
		print("Oops! Jogada inválida! Tente de novo.")
		n = int(input("Quantas peças você vai tirar? "))
	if n == 1:
		print("Você tirou uma peça.")
	else:
		print("Você tirou %d peças." % (n))

	return n

def partida():
	npecas = int(input("Quantas peças? "))
	while npecas < 1:
		print("Ops, houve um erro. Tente de novo.")
		npecas = int(input("Quantas peças? "))
	...
	if npecas % (limite + 1) != 0:
		print("\nComputador começa!\n")
		npecas -= computador_escolhe_jogada(npecas, limite)
		if npecas == 0:
			print("\nFim do jogo! O computador ganhou!\n")
			return COMPUTADOR
		else:
	...
	while True:
		npecas -= usuario_escolhe_jogada(npecas, limite)
		if npecas == 0:
			print("\nFim do jogo! Parabéns, você ganhou!\n")
			return JOGADOR
		else:
			if npecas == 1:
				print("Agora resta apenas uma peça no tabuleiro.\n")
...
def imprime_placar(placar):
	print("\nPlacar: Você %d X %d Computador\n" % (placar[JOGADOR], placar[COMPUTADOR]))
